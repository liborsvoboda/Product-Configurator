...
class Securimage_send {
	 ...
	 * @var string
	 */
	var $background_directory = '/captcha/backgrounds/bg3.jpg'; //'./backgrounds';
	...
	*/
	...
	function Securimage_send()
	{
		...
		$this->image_bg_color   = new Securimage_send_Color(0xff, 0xff, 0xff);
    $this->text_color       = new Securimage_send_Color(0x3d, 0x3d, 0x3d);
		$this->multi_text_color = array(new Securimage_send_Color(0x0, 0x20, 0xCC),
																		new Securimage_send_Color(0x0, 0x30, 0xEE),
																		new Securimage_send_Color(0x0, 0x40, 0xCC),
																		new Securimage_send_Color(0x0, 0x50, 0xEE),
																		new Securimage_send_Color(0x0, 0x60, 0xCC));
		...
		$this->num_lines            = 0;
		$this->line_color           = new Securimage_send_Color(0x3d, 0x3d, 0x3d);
		$this->draw_lines_over_text = true;

		$this->image_signature = '';
		$this->signature_color = new Securimage_send_Color(0x20, 0x50, 0xCC);
		...
		$this->sqlite_database = 'database/securimage-send.sqlite';
	...
	function outputAudioFile()
	{
		...
		header("Content-Disposition: attachment; filename=\"securimage_audio_send.{$ext}\"");
	...
	function saveData()
	{
		$_SESSION['securimage_send_lcode_value'] = strtolower($this->code);
		$_SESSION['securimage_send_lcode_ctime'] = time();
	...
	function validate()
	{
		...
		if (isset($_SESSION['securimage_send_lcode_value']) && trim($_SESSION['securimage_send_lcode_value']) != '') {
			if ($this->isCodeExpired($_SESSION['securimage_send_lcode_ctime']) == false) {
			  $code = $_SESSION['securimage_send_lcode_value'];
			}
		...
		if ($code != '') {
			if ($code == $code_entered) {
			  ...
			  $_SESSION['securimage_send_lcode_value'] = '';
			  $_SESSION['securimage_send_lcode_ctime'] = '';
	...
	function getCode()
	{
		if (isset($_SESSION['securimage_send_lcode_value']) && !empty($_SESSION['securimage_send_lcode_value'])) {
			return strtolower($_SESSION['securimage_send_lcode_value']);
		} else {
	...
	function saveCodeToDatabase()
	{
		...
		if ($this->use_sqlite_db && $this->sqlite_handle !== false) {
			$ip = $this->getIPHash();
			$time = time();
			$code = $_SESSION['securimage_send_lcode_value']; // hash code for security - if cookies are disabled the session still exists at this point
...
class Securimage_send_Color {
	 ...
	 */
	function Securimage_send_Color($red, $green = null, $blue = null)
	{
