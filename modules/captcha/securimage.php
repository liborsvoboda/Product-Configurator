...
class Securimage {
	 ...
	 * @var string
	 */
	var $background_directory = './background'; //'./backgrounds';
	...
	*/
    ...
    $this->text_color       = new Securimage_Color(0x3d, 0x3d, 0x3d);
        ...
        $this->background_transparency_percentage = 100;
		$this->num_lines            = 0;
	...
	function allocateColors()
	{
        ...
        $alpha1 = intval($this->background_transparency_percentage / 100 * 127);
		$this->gdbgcolor = imagecolorallocatealpha($this->im, $this->image_bg_color->r, $this->image_bg_color->g, $this->image_bg_color->b, $alpha1);
	...
	function saveData()
	{
		$_SESSION['securimage_lcode_value'] = strtolower($this->code);
		$_SESSION['securimage_lcode_ctime'] = time();
	...
	function validate()
	{
		...
		if (isset($_SESSION['securimage_lcode_value']) && trim($_SESSION['securimage_lcode_value']) != '') {
			if ($this->isCodeExpired($_SESSION['securimage_lcode_ctime']) == false) {
			  $code = $_SESSION['securimage_lcode_value'];
			}
		...
		if ($code != '') {
			if ($code == $code_entered) {
			  ...
			  $_SESSION['securimage_lcode_value'] = '';
			  $_SESSION['securimage_lcode_ctime'] = '';
	...
	function getCode()
	{
		if (isset($_SESSION['securimage_lcode_value']) && !empty($_SESSION['securimage_lcode_value'])) {
			return strtolower($_SESSION['securimage_lcode_value']);
		} else {
	...
	function saveCodeToDatabase()
	{
		...
		if ($this->use_sqlite_db && $this->sqlite_handle !== false) {
			$ip = $this->getIPHash();
			$time = time();
			$code = $_SESSION['securimage_lcode_value']; // hash code for security - if cookies are disabled the session still exists at this point
